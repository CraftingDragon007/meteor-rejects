...
import anticope.rejects.mixin.CameraAccessor;
...
public class SkeletonESP extends Module {
    ...
    private Vec3d getEntityRenderPosition(Entity entity, double partial) {
        double x = entity.lastX + ((entity.getX() - entity.lastX) * partial) - ((CameraAccessor)mc.getEntityRenderDispatcher().camera).getPos().x;
        double y = entity.lastY + ((entity.getY() - entity.lastY) * partial) - ((CameraAccessor)mc.getEntityRenderDispatcher().camera).getPos().y;
        double z = entity.lastZ + ((entity.getZ() - entity.lastZ) * partial) - ((CameraAccessor)mc.getEntityRenderDispatcher().camera).getPos().z;
        return new Vec3d(x, y, z);
    }

    private Color getColorFromDistance(Entity entity) {
        Vec3d entityPos = new Vec3d(entity.getX(), entity.getY(), entity.getZ());
        double distance = ((CameraAccessor)mc.gameRenderer.getCamera()).getPos().distanceTo(entityPos);
